import EdensConjectureDefinitions
...
/-!
# Counterexample constructions for Eden's conjecture

`TorusCounterexample` retains the earlier construction on an open product of
annuli. It does not establish global attraction on the whole Hilbert space
required by the thesis formulation.

`PolynomialCounterexample` develops the supplied polynomial construction on
R^5. Its algebraic spectrum calculations must still be connected to the
derivative of the actual global flow before they refute the conjectures.
This file imports only definitions, never the admitted negation targets.
-/
...
/-! ## The polynomial counterexample supplied on 12 September 2026

This construction is distinct from `TorusCounterexample`: its phase space is
all of R^5 and its proposed global attractor is a product of closed disks.
The lemmas below establish algebraic and spectral calculations, not yet the
existence, differentiability, or global attraction of its solution operator.
-/
...
lemma localLyapunovDimension_bddAbove {n : ℕ} (D : C1DynamicalSystem n) (A : Set (PhaseSpace n)) :
    BddAbove (localLyapunovDimension D '' A) := by
  refine ⟨n, ?_⟩
  rintro d ⟨x, _, rfl⟩
  exact kaplanYorkeDimension_le n (D.upperLE x)
...
lemma torus_localLyapunovDimension {x : X} (hx : x ∈ T) :
  ...
  unfold localLyapunovDimension
  rw [torus_upperLE hx, spectrum_BB_dimension]
...
lemma equilibrium_local_dimension (r s : RadialType) {a b : ℂ} (ha : ‖a‖ = 1) (hb : ‖b‖ = 1) :
  ...
  unfold localLyapunovDimension
  rw [equilibrium_upperLE r s ha hb]
...
theorem modern_no_attainment :
    ¬ ∃ x ∈ K, (StationarySolution system.φ x ∨ system.UnstablePeriodicOrbit x) ∧
      Orbit system.φ x ⊆ K ∧ localLyapunovDimension system x = sSup (localLyapunovDimension system '' K) := by
  apply no_modern_attainment_of_gap system K 3 (localLyapunovDimension_bddAbove system K)
  · obtain ⟨x, hx⟩ := nonempty_T
    refine ⟨x, T_subset_K hx, ?_⟩
    rw [torus_localLyapunovDimension hx]
    norm_num
  · intro x _ hx
    ...
    ¬ (∀ n : ℕ, 0 < n → ∀ D : C1DynamicalSystem n,
      ∀ A : Set (PhaseSpace n), D.GlobalAttractor A →
        ∃ x ∈ A, (StationarySolution D.φ x ∨ D.UnstablePeriodicOrbit x) ∧ Orbit D.φ x ⊆ A ∧
          localLyapunovDimension D x = sSup (localLyapunovDimension D '' A)) := by
  intro h
